...
# adjust each different loss
class ablation_study():
    ...
    def g_train_step(self, low_images, high_images, train=True):
        with tf.GradientTape() as tape:
            ...
            total_loss = lrec_d + lrec_g + ladv_g
...
def ablation_study_test(kernel=(7, 7)):
    # Overall structure
    global ablation_study_lrec_ladv_enc
    global ablation_study_lrec_ladv_reg
    global ablation_study_lrec_ladv_gen

    global ablation_study_lrec_ladv_lreg_enc
    global ablation_study_lrec_ladv_lreg_reg
    global ablation_study_lrec_ladv_lreg_gen

    global ablation_study_lrec_ladv_lreg_ldis_enc
    global ablation_study_lrec_ladv_lreg_ldis_reg
    global ablation_study_lrec_ladv_lreg_ldis_gen

    ablation_study_lrec_ladv_lreg_ldis_enc = encoder()
    ablation_study_lrec_ladv_lreg_ldis_reg = regression()
    ablation_study_lrec_ladv_lreg_ldis_gen = generator()

    ablation_study_lrec_ladv_lreg_enc = encoder()
    ablation_study_lrec_ladv_lreg_reg = regression()
    ablation_study_lrec_ladv_lreg_gen = generator()

    ablation_study_lrec_ladv_enc = encoder()
    ablation_study_lrec_ladv_reg = regression()
    ablation_study_lrec_ladv_gen = generator()

    # encoder.load_weights('/disk2/bosen/CDRG-SR/weights/encoder')
    # reg.load_weights('/disk2/bosen/CDRG-SR/weights/reg_x_cls_REG')
    # generator.load_weights('weights/unet_generator_20_step4')
    # ablation_study_lrec_ladv_ldis_gen.load_weights('weights/ablation_study_reg_generator')
    ...
    ablation_study_lrec_ladv_gen.load_weights('weights/ablation_study_lrec_ladv_generator')
    ...
    ablation_study_lrec_ladv_lreg_gen.load_weights('weights/ablation_study_lrec_ladv_lreg_generator')

    ablation_study_lrec_ladv_lreg_ldis_enc.load_weights('weights/ablation_study_lrec_ladv_lreg_ldis_encoder')
    ablation_study_lrec_ladv_lreg_ldis_reg.load_weights('weights/ablation_study_lrec_ladv_lreg_ldis_reg')
    ablation_study_lrec_ladv_lreg_ldis_gen.load_weights('weights/ablation_study_lrec_ladv_lreg_ldis_generator')

    lrec_ladv_mpsnr, lrec_ladv_mssim = [[] for i in range(6)], [[] for i in range(6)]
    lrec_ladv_ldis_mpsnr, lrec_ladv_ldis_mssim = [[] for i in range(6)], [[] for i in range(6)]
    total_mpsnr, total_mssim = [[] for i in range(6)], [[] for i in range(6)]
    path = '/disk2/bosen/Datasets/AR_test/'

    for id_type in os.listdir(path):
        print(id_type)
        plt.subplots(figsize=(6, 4))
        plt.subplots_adjust(wspace=0, hspace=0)
        for id_num, filename in enumerate(os.listdir(path + id_type)):
            if id_num == 20:
                break
            image = cv2.imread(path + id_type + '/' + filename, 0) / 255
            id = int(id_type[2:]) + 90
            ...
            low2_image = cv2.resize(cv2.resize(blur_gray, (20, 20), cv2.INTER_CUBIC), (64, 64),cv2.INTER_CUBIC).reshape(1, 64, 64, 1)
            low3_image = cv2.resize(cv2.resize(blur_gray, (16, 16), cv2.INTER_CUBIC), (64, 64),cv2.INTER_CUBIC).reshape(1, 64, 64, 1)
            low4_image = cv2.resize(cv2.resize(blur_gray, (10, 10), cv2.INTER_CUBIC), (64, 64),cv2.INTER_CUBIC).reshape(1, 64, 64, 1)
            low5_image = cv2.resize(cv2.resize(blur_gray, (8, 8), cv2.INTER_CUBIC), (64, 64), cv2.INTER_CUBIC).reshape(1, 64, 64, 1)
            ...
            # 2 row
            for num, low in enumerate([image, low1_image, low2_image, low3_image, low4_image, low5_image]):
                z, f1_e, f2_e, f3_e, f4_e = ablation_study_lrec_ladv_enc(tf.reshape(low, [1, 64, 64, 1]))
                _, _, z = ablation_study_lrec_ladv_reg(z)
                syn_woreg, _, _, _ = ablation_study_lrec_ladv_gen([z, f1_e, f2_e, f3_e, f4_e])
                lrec_ladv_mpsnr[num].append(tf.image.psnr(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                lrec_ladv_mssim[num].append(tf.image.ssim(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                if id_num == 0:
                    plt.subplot(4, 6, num + 7)
                    plt.axis('off')
                    plt.imshow(tf.reshape(syn_woreg, [64, 64]), cmap='gray')

            # 3 row
            for num, low in enumerate([image, low1_image, low2_image, low3_image, low4_image, low5_image]):
                z, f1_e, f2_e, f3_e, f4_e = ablation_study_lrec_ladv_lreg_enc(tf.reshape(low, [1, 64, 64, 1]))
                _, _, z = ablation_study_lrec_ladv_lreg_reg(z)
                syn_woreg, _, _, _ = ablation_study_lrec_ladv_lreg_gen([z, f1_e, f2_e, f3_e, f4_e])

                lrec_ladv_ldis_mpsnr[num].append(tf.image.psnr(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                lrec_ladv_ldis_mssim[num].append(tf.image.ssim(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                if id_num == 0:
                    plt.subplot(4, 6, num + 13)
                    plt.axis('off')
                    plt.imshow(tf.reshape(syn_woreg, [64, 64]), cmap='gray')

            # 4 row
            for num, low in enumerate([image, low1_image, low2_image, low3_image, low4_image, low5_image]):
                z, f1_e, f2_e, f3_e, f4_e = ablation_study_lrec_ladv_lreg_ldis_enc(tf.reshape(low, [1, 64, 64, 1]))
                _, _, z = ablation_study_lrec_ladv_lreg_ldis_reg(z)
                syn_woreg, _, _, _ = ablation_study_lrec_ladv_lreg_ldis_gen([z, f1_e, f2_e, f3_e, f4_e])
                total_mpsnr[num].append(tf.image.psnr(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                total_mssim[num].append(tf.image.ssim(tf.cast(tf.reshape(image, [1, 64, 64, 1]), dtype=tf.float32), tf.cast(syn_woreg, dtype=tf.float32), max_val=1)[0])
                if id_num == 0:
                    plt.subplot(4, 6, num + 19)
                    plt.axis('off')
                    plt.imshow(tf.reshape(syn_woreg, [64, 64]), cmap='gray')

        plt.savefig(f'result/compare_experiment/compare_result_ID{id}')
        plt.close()

    lrec_ladv_mpsnr, lrec_ladv_mssim = tf.reduce_mean(lrec_ladv_mpsnr, axis=-1), tf.reduce_mean(lrec_ladv_mssim, axis=-1)
    lrec_ladv_ldis_mpsnr, lrec_ladv_ldis_mssim = tf.reduce_mean(lrec_ladv_ldis_mpsnr, axis=-1), tf.reduce_mean(lrec_ladv_ldis_mssim, axis=-1)
    total_mpsnr, total_mssim = tf.reduce_mean(total_mpsnr, axis=-1), tf.reduce_mean(total_mssim, axis=-1)

    print(lrec_ladv_mpsnr)
    print(lrec_ladv_mssim)
    print(' ')
    print('-----------')
    print(lrec_ladv_ldis_mpsnr)
    print(lrec_ladv_ldis_mssim)
    print(' ')
    print('-----------')
    print(total_mpsnr)
    print(total_mssim)
...
if __name__ == '__main__':
    ...
    ablation_stydy = ablation_study(epochs=10, step=4)
    ablation_stydy.training(train=False)

    ablation_study_test(kernel=(7, 7))
